...
import { ChevronRight, ChevronLeft, Check, Calendar, User, Clock, Video, Share2, ExternalLink, Loader, Stethoscope } from 'lucide-react';
import Card from '../../components/ui/Card';
import Button from '../../components/ui/Button';
import toast from 'react-hot-toast';
import { useNavigate } from 'react-router-dom';
import { shareToPatientWhatsApp, shareToDocterWhatsApp } from '../../utils/meetLink';
import { useAuthStore } from '../../store/authStore';

const TIME_SLOTS = ['09:00','10:00','11:00','12:00','14:00','15:00','16:00','17:00'];

function generateSlots(doctorId: string, date: Date) {
  const seed = doctorId.split('').reduce((a, c) => a + c.charCodeAt(0), 0) + date.getDate();
  return TIME_SLOTS.map((t, i) => ({
    id: `${doctorId}-${date.toDateString()}-${i}`,
    time: t,
    is_booked: ((seed + i * 7) % 10) < 3,
  }));
}
...
export default function BookAppointment() {
  ...
  // Realtime verified doctors (updates instantly when admin approves)
  useEffect(() => {
  ...
  const slots = selectedDoctor ? generateSlots(selectedDoctor.id, selectedDate) : [];
  ...
  async function confirm() {
    ...
    try {
      ...
      const { patientSent, doctorSent } = result.emails;
      ...
      if (patientSent && doctorSent) {
        toast.success('Appointment confirmed! Confirmation emails sent to you and the doctor.');
      } else if (patientSent || doctorSent) {
        toast.success('Appointment booked! Confirmation email sent to one party.');
      } else {
        toast.success('Appointment booked! Share the Meet link below if emails did not arrive.');
      }
      ...
      <div>
        ...
        <Card style={{ maxWidth: 520 }}>
          <div style={{ textAlign: 'center', marginBottom: '1.75rem' }}>
            <div style={{ width: 64, height: 64, borderRadius: '50%', background: 'var(--accent)', margin: '0 auto 1rem', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <Check size={28} color="var(--primary)" />
            </div>
            <h4 style={{ marginBottom: '0.25rem' }}>Booking Confirmed!</h4>
            <p style={{ fontSize: 'var(--font-size-sm)', color: 'var(--text-muted)' }}>Your consultation has been scheduled successfully.</p>
          </div>
          ...
          <div style={{ background: 'var(--accent)', border: '1px solid rgba(29,158,117,0.25)', borderRadius: 'var(--radius-md)', padding: '1rem', marginBottom: '1.25rem' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.625rem' }}>
              <Video size={16} color="var(--primary)" />
              <span style={{ fontWeight: 700, fontSize: 'var(--font-size-sm)', color: 'var(--primary)' }}>Your Google Meet Room</span>
            </div>
            <p style={{ fontFamily: 'monospace', fontSize: '0.85rem', fontWeight: 700, color: 'var(--text-primary)', wordBreak: 'break-all', marginBottom: '0.75rem' }}>
              {booked.meet_link}
            </p>
            <div style={{ display: 'flex', gap: '0.625rem', flexWrap: 'wrap' }}>
              <a href={booked.meet_link} target="_blank" rel="noopener noreferrer" style={{ flexShrink: 0 }}>
                <Button size="sm" icon={<ExternalLink size={13} />}>Open Meet</Button>
              </a>
              <Button size="sm" variant="ghost" icon={<Share2 size={13} />}
          ...
          <div style={{ display: 'flex', gap: '0.75rem', flexWrap: 'wrap' }}>
            <Button fullWidth onClick={() => shareToPatientWhatsApp(booked.doctor_name, booked.slot_datetime, booked.meet_link, user?.phone)}
              icon={<Share2 size={16} />}
            ...
            <Button fullWidth variant="ghost" onClick={() => navigate('/patient/appointments')}>
    ...
    <div>
        ...
        {[{n:1,label:'Select Doctor'},{n:2,label:'Choose Slot'},{n:3,label:'Confirm'}].map(({n,label},i,arr) => (
          <div key={n} style={{ display: 'flex', alignItems: 'center', flex: n < arr.length ? 1 : 'none' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', flexShrink: 0 }}>
              <div style={{ width: 32, height: 32, borderRadius: '50%', background: step >= n ? 'var(--primary)' : 'var(--neutral)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                {step > n ? <Check size={14} color="white" /> : <span style={{ color: step === n ? 'white' : 'var(--text-muted)', fontSize: 'var(--font-size-sm)', fontWeight: 700 }}>{n}</span>}
              </div>
        ...
        <div>
          {loadingDoctors ? (
            ...
            <Card style={{ textAlign: 'center', padding: '3rem' }}>
              <Stethoscope size={48} style={{ marginBottom: '1rem', opacity: 0.4, color: 'var(--text-muted)' }} />
              <h5 style={{ marginBottom: '0.5rem' }}>No Doctors Available Yet</h5>
              <p style={{ color: 'var(--text-muted)', fontSize: 'var(--font-size-sm)' }}>
                No verified dermatologists are registered on DERMA AI yet.<br />
                  ...
                  <Card key={doc.id} hover style={{ cursor: 'pointer', border: selectedDoctor?.id === doc.id ? '2px solid var(--primary)' : '0.5px solid var(--border)' }}
                    ...
                    <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center', marginBottom: '0.75rem' }}>
                      {doc.avatar_url ? (
                        ...
                        <div style={{ width: 48, height: 48, borderRadius: '50%', background: 'var(--accent)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, color: 'var(--primary)', fontSize: '1.1rem', flexShrink: 0 }}>
                          {doc.name.charAt(0)}
                        </div>
                      )}
                      <div style={{ flex: 1 }}>
                        ...
                        <p style={{ fontSize: 'var(--font-size-xs)', color: 'var(--text-muted)' }}>
                          {doc.city}{doc.experience_years > 0 ? ` · ${doc.experience_years} yrs exp.` : ''}
                        </p>
                      </div>
                    ...
                    <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
                      ...
                      {doc.hospital && (
                        <span style={{ fontSize: '0.7rem', color: 'var(--text-muted)', padding: '0.2rem 0.625rem', background: 'var(--neutral)', borderRadius: 'var(--radius-full)' }}>
                          {doc.hospital}
                        </span>
                      )}
                    </div>
        ...
        <Card style={{ maxWidth: '640px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1.25rem' }}>
            {selectedDoctor.avatar_url ? (
              ...
              <div style={{ width: 44, height: 44, borderRadius: '50%', background: 'var(--accent)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700, color: 'var(--primary)', flexShrink: 0 }}>
          ...
          <div style={{ display: 'flex', gap: '0.75rem' }}>
            ...
            <Button iconRight={<ChevronRight size={16} />} onClick={() => setStep(3)} disabled={!selectedSlot}>Next: Confirm</Button>
          </div>
        </Card>
      )}
      ...
      {step === 3 && selectedDoctor && (
        <Card style={{ maxWidth: '480px' }}>
          ...
          <div style={{ background: 'var(--accent)', border: '0.5px solid rgba(29,158,117,0.3)', borderRadius: 'var(--radius-sm)', padding: '0.75rem 1rem', marginBottom: '1.25rem', fontSize: 'var(--font-size-xs)', color: 'var(--secondary)' }}>
